...
def dataframe_plot(
    ...
    error_color = np.array([0.5, 0.5, 0.5])
    ...
    for i, c in enumerate(df.columns):
        try:
            if len(df[c].unique()) < np.ceil(df.shape[0] / 10):
                ...
                img[
                    df[c].isin(
                        counts[
                            counts
                            < rare_category_factor * df.shape[0] / counts.shape[0]
                        ].index
                    ),
                    i,
                ] = rare_colour
            ...
            raise Warning(
                'Column %s caused exception: %s with message "%s". Setting column to solid grey.'
                % (str(c), str(e), str(e.message))
            )
            img[:, i] = error_color
    ...
    if df.shape[0] / (fig.dpi * rows_per_pixel) > 2 ** 14:
        raise ValueError(
            ...
            % (rows_per_pixel, fig.dpi, rows_per_pixel, df.shape[0])
        )
    fig.set_size_inches(
        0.3 * df.shape[1],
        max(4, min(2 ** 14, df.shape[0] / (fig.dpi * rows_per_pixel))),
    )
